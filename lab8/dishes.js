...
function mapDish(apiDish) {
  ...
  const id = Number(pick(apiDish, ["id"], 0));
  ...
  return { id, keyword, category, kind, name, count, price, image };
}
...
async function loadDishes() {
  ...
  return mapped.filter(d => Number.isFinite(d.id) && d.id > 0 && d.keyword && d.category);
}
